...
self.FGL_MAP_MENU:Hide()
    ...
    icon.OnUpdate = function(self, elapsed)
		if self.dragging then
			...
			if falsetonil(FGL.db.minimapiconfree) then
				local free = FindGroupDB.minimapposition_free;
				free.point, free.x, free.y = getFreePoints(px, py);
			else
				px, py = px / scale, py / scale;
				FindGroupDB.minimapposition = math.deg(math.atan2(py - my, px - mx)) % 360;
			end
			self:UpdatePosition();
		end
    end
	...
	if not FindGroupDB.minimapposition then FindGroupDB.minimapposition = random(0, 360) end
            local angle = math.rad(FindGroupDB.minimapposition)
	...
	if not FindGroupDB.minimapposition_free then FindGroupDB.minimapposition_free = {} end
            FindGroupDB.minimapposition_free.point, FindGroupDB.minimapposition_free.x, FindGroupDB.minimapposition_free.y = getFreePoints((self:GetLeft() + self:GetWidth()/2)*scale, (self:GetTop() - self:GetHeight()/2)*scale);
      end
    end
    icon:Load();

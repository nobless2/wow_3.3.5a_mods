...
FindGroupDB.usingtime=FindGroupDB.usingtime+elapsed
...
for i=1, FGL.db.nummsgsmax do
	...
	if FGL.db.lastmsg[i] then
		...
		if (timeleftnow > FGL.db.timeleft ) then
			...
			if timeleftnow >= 3 then if FGL.db.lastmsg[i][7] == string.format("%02x%02x%02x",0.63*255,0.63*255,0.63*255) then FindGroup_setmsgtext(i) end
		end

local MAJOR, MINOR = "LibFail-1.0", tonumber("202") or 999
...
--@field 35 Fail_Gorehowl_FireBomb
...
function lib:SPELL_DAMAGE(timestamp, type, sourceGUID, sourceName, sourceFlags, destGUID, destName, destFlags, spellId, spellName, spellSchool, damage, overkill)
    ...
    -- Lady Deathwhisper Vengeful Shades (This spell only has a 5 yard range so it's still completely avoidable by staying away from shades.)
...
--]]
    ...
    -- Northrend Beasts - Gorehowl - Fire Bomb
    if spellId == 66317 and is_playerevent then -- the initial damage from whence you have 2 seconds to move
        self.LastEvent.Fail_Gorehowl_FireBomb[destName] = timestamp

        return
    elseif (spellId == 67472 or spellId == 66320 or spellId == 67475 or spellId == 67473) and is_playerevent then
        if not self.LastEvent.Fail_Gorehowl_FireBomb[destName] then
            -- no initial damage, so the failer managed to go into the flame on the ground, good job
            self.LastEvent.Fail_Gorehowl_FireBomb[destName] = timestamp
        end
        if (timestamp - self.LastEvent.Fail_Gorehowl_FireBomb[destName]) > 2 then
            self:FailEvent("Fail_Gorehowl_FireBomb", destName, self.FAIL_TYPE_NOTMOVING)
            self.LastEvent.Fail_Gorehowl_FireBomb[destName] = timestamp -- so as not to spam
        end
...
--12/26 20:27:36.560  SWING_DAMAGE,0xF13000966C005B92,"Blood Beast",0x10a48,0x040000000396C708,"Outofcontrol",0x2000514,5033,0,1,0,0,1258,nil,nil,nil
    ...
    -- Tanks exempt though because a tank isn't going to kite the boss or adds around room just to dodge these
    if self:GetMobId(sourceGUID) == 38222 and is_playerevent and overkill > 0 then
        self:FailEvent("Fail_Deathwhisper_Shade", destName, self.FAIL_TYPE_MOVING)
...
function lib:SPELL_DISPEL(timestamp, type, sourceGUID, sourceName, sourceFlags, destGUID, destName, destFlags, spellId, spellName, spellSchool, extraSpellId)
